...
class BinarySearchTree {
  ...
  insert(info) {
    let subTree = new BinarySearchTree(info);
    if (info < this.value) {
      if (!this.left) this.left = subTree;
      else this.left.insert(info);
    } else {
      if (!this.right) this.right = subTree;
  ...
  contains(search) {
    ...
    if (search < this.value) {
      ...
      else return this.left.contains(search);
    } else {
      if (!this.right) return false;
      else return this.right.contains(search);
    }
  }
  depthFirstForEach(cb, texto) {
